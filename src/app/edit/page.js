...
async function getQuestionsAndAnswers(test) {
	...
	for (const question of questions) {
		const questionAndAnswers = await pb
			.collection("questions")
			.getOne(question.id, { expand: "answers" });
		questionsAndAnswers.push({
			question: questionAndAnswers.question,
			answers: questionAndAnswers.expand
				? questionAndAnswers.expand.answers
				: [],
		});
	}
...
export default async function Test() {
	...
	return <EditPage test={test} questionsAndAnswers={questionsAndAnswers} />;
}
